...
无论是 `cargo run` 还是 `cargo run --release`，默认工作目录都是 `web-sites-hub/backend-resume`。因此：

- SQLite 数据库位于 `web-sites-hub/backend-resume/invites.db`
- 模板目录：`web-sites-hub/backend-resume/templates`
- 静态资源目录：`web-sites-hub/backend-resume/static`
