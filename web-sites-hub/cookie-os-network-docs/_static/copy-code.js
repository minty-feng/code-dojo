            ...
            mutations.forEach(function(mutation) {
                mutation.addedNodes.forEach(function(node) {
                    ...
                    if (node.nodeType === 1 && node.querySelectorAll) {
                        // 检查节点本身是否是代码块
                        if (node.matches && node.matches('.highlight, .highlighttable, div[class*="highlight"]')) {
                            // 排除行内代码
                            if (!node.closest('p, li, td, th')) {
                                addCopyButton(node);
                            }
                        }
                        // 查找子元素中的代码块
                        const codeBlocks = node.querySelectorAll('.highlight, .highlighttable, div[class*="highlight"]');
                        codeBlocks.forEach(function(block) {
                            // 排除行内代码
                            if (!block.closest('p, li, td, th')) {
                                addCopyButton(block);
