...
PACKAGE_NAME="os-network-docs-nginx-${TIMESTAMP}.tar.gz"
...
# 拷贝部署脚本（如果存在）
if [ -f "deploy-os-network.sh" ]; then
    echo "📋 添加部署脚本..."
    cp deploy-os-network.sh "$TEMP_DIR/deploy-os-network.sh"
fi
...
2. 运行部署脚本（自动解压和部署）：
   ...
   sudo bash deploy-os-network.sh os-network-docs-nginx-*.tar.gz
...
4. 访问地址：
   ...
   HTTPS: https://blog.joketop.com/os（如果启用 HTTPS）

配置文件位置：
- Nginx 配置: /etc/nginx/sites-available/docs-code-dojo (统一配置)
- 部署目录: /var/www/html/cookie-os-network
...
if [ -f "deploy-os-network.sh" ]; then
    tar --format=ustar -czf "../$PACKAGE_NAME" html README.txt deploy-os-network.sh
else
