...
PACKAGE_NAME="ds-core-docs-nginx-${TIMESTAMP}.tar.gz"
...
# 拷贝部署脚本（如果存在）
if [ -f "deploy-ds-core.sh" ]; then
    echo "📋 添加部署脚本..."
    cp deploy-ds-core.sh "$TEMP_DIR/deploy-ds-core.sh"
fi
...
2. 运行部署脚本（自动解压和部署）：
   ...
   sudo bash deploy-ds-core.sh ds-core-docs-nginx-*.tar.gz
...
if [ -f "deploy-ds-core.sh" ]; then
    tar --format=ustar -czf "../$PACKAGE_NAME" html README.txt deploy-ds-core.sh
else
...
echo "💡 提示："
echo "   1. 上传到服务器"
echo "   2. 解压部署: sudo bash deploy-ds-core.sh $PACKAGE_NAME"
echo "   3. 配置 Nginx: sudo bash deploy-all-docs.sh [--letsencrypt --email your@email.com]"
echo "   4. 访问: http://blog.joketop.com/ds (或 https:// 如果启用)"
