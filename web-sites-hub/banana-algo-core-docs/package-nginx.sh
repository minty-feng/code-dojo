...
PACKAGE_NAME="algo-core-docs-nginx-${TIMESTAMP}.tar.gz"
...
# 拷贝部署脚本（如果存在）
if [ -f "deploy-algo-core.sh" ]; then
    echo "📋 添加部署脚本..."
    cp deploy-algo-core.sh "$TEMP_DIR/deploy-algo-core.sh"
fi
...
2. 运行部署脚本（自动解压和部署）：
   ...
   sudo bash deploy-algo-core.sh algo-core-docs-nginx-*.tar.gz
...
4. 访问地址：
   HTTP:  http://blog.joketop.com/algo
   HTTPS: https://blog.joketop.com/algo（如果启用 HTTPS）

配置文件位置：
- Nginx 配置: /etc/nginx/sites-available/docs-code-dojo (统一配置)
- 部署目录: /var/www/html/banana-algo-core
- Let's Encrypt 证书: /etc/letsencrypt/live/blog.joketop.com/

注意：
- Nginx 配置由统一脚本 deploy-all-docs.sh 管理
- 部署脚本 deploy-algo-core.sh 只负责解压和拷贝文件
EOF
...
if [ -f "deploy-algo-core.sh" ]; then
    tar --format=ustar -czf "../$PACKAGE_NAME" html README.txt deploy-algo-core.sh
else
...
echo "💡 提示："
echo "   1. 上传到服务器"
echo "   2. 解压部署: sudo bash deploy-algo-core.sh $PACKAGE_NAME"
echo "   3. 配置 Nginx: sudo bash deploy-all-docs.sh [--letsencrypt --email your@email.com]"
echo "   4. 访问: http://blog.joketop.com/algo (或 https:// 如果启用)"
